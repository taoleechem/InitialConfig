...
void Molecule::ToPDBfile(const string filename, int connection_matrix[][8])
{
	...
	for (int i = 0; i != number; i++)
	{
		tofile << "HETATM    " << i+1 << "	 " << name[i] << "   LIG     1      " << corr[i][0] << "  " << corr[i][1] << "  " << corr[i][2] << "  1.00  0.00           " << name[i] << endl;
	}
	for (int i = 0; i != number; i++)
	{
		tofile << "CONECT    "<<i+1;
...
void Molecule::ToPDBfileOnlyGeo(ofstream &tofile, int initial_label)
{
	for (int i = 0; i != number; i++)
	{
		tofile << "HETATM\t\t" << initial_label+i+1 << "\t\t" << name[i] << "\tLIG\t\t1\t\t" <<fixed<<setprecision(4)<< corr[i][0] << "\t" << corr[i][1] << "\t" << corr[i][2] << "\t1.00  0.00           " << name[i] << endl;
		cout << "HETATM\t\t" << initial_label + i + 1 << "\t\t" << name[i] << "\tLIG\t\t1\t\t" << fixed << setprecision(4) <<corr[i][0] << "\t" << corr[i][1] << "\t" << corr[i][2] << "\t1.00  0.00           " << name[i] << endl;
	}
}
void ToPDBfileAmBnType(const string filename, vector<Molecule> &A, vector<Molecule> &B, int connect_m[][8], int connect_n[][8])
{
	...
	tofile << "COMPND\t\tRESULT" << endl;
	cout << "COMPND\t\tRESULT" << endl;
	tofile << "AUTHOR\t\tGENERATED BY " << "Tao" << endl;
	cout << "AUTHOR\t\tGENERATED BY " << "Tao" << endl;
